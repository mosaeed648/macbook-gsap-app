...
export default function MacbookModel14(props) {
  ...
  // 🧠 نخزن الـ Meshes اللي يمكن تغيير لونها مرة واحدة
  const colorableMeshes = useRef([]);

  useEffect(() => {
    scene.traverse((child) => {
      if (child.isMesh && !noChangeParts.includes(child.name)) {
        colorableMeshes.current.push(child);
      }
    });
  }, [scene]);

  // 🎨 تحديث اللون فقط للعناصر المطلوبة
  useEffect(() => {
    colorableMeshes.current.forEach((mesh) => {
